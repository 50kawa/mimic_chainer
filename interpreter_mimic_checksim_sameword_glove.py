...
MODEL_PATH = './data/glove_34_most.model'
...
def interpreter(model_path):
    ...
    corpus=Mimicdata()
    corpus.load_interpreter("datapack_GloVe.pkl")
    ...
    vector_dict={}
    glove_model = KeyedVectors.load_word2vec_format('D:/glove.840B.300d/glove.840B.300d.read.txt', binary=False,
                                              unicode_errors="ignore")
    with open("../wordlist_orderedbyfreq.pkl", "rb") as f:
        word_list = pickle.load(f)  # len2176135?
    ...
    for i,word in enumerate(word_list):
        ...
        print(cos_sim(vector.data[0],glove_model[word]))
        if i>1000:
            break

    with open("mimic_vectordict.pkl","wb") as f:
