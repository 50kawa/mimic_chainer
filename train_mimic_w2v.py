...
def main():
    ...
    for num, epoch in enumerate(range(n_epoch)):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            serializers.save_hdf5('data/w2v_' + str(epoch) + '.model', model)
            serializers.save_hdf5('data/w2v_' + str(epoch) + '.state', optimizer)
    ...
    # save loss data
    with open('./data/loss_train_data_w2v.pkl', 'wb') as f:
        pickle.dump(train_loss_data, f)
    with open('./data/loss_test_data_w2v.pkl', 'wb') as f:
